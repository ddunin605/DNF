(() => {
  ...
  style.textContent = `
    ...
    #${PANEL_ID} .grid { display:grid; grid-template-columns: 1fr 1fr; gap: 8px; }
    #${PANEL_ID} .small { font-size: 12px; opacity:.8; margin-top:6px; }
    #${PANEL_ID} .row { display:flex; gap:8px; flex-wrap: wrap; margin-top:6px; }
    #${PANEL_ID} .chip { display:inline-flex; align-items:center; gap:6px; padding:6px 8px; border-radius:8px; border:1px solid rgba(255,255,255,.18); cursor:pointer; user-select:none; }
    #${PANEL_ID} .chip input { margin:0; }
  ...
  // ==== STATE / PERSIST ======================================================
  const SS_KEY = 'ddunin_filter_panel_state_v1';
  const loadState = () => {
  ...
  const parseFameExpr = (expr) => {
    ...
    // allow ">50000 <60000", ">=50000 <=60000", "50000-60000", "60000" etc.
    const ops = [];
  ...
  const filterRows = () => {
    ...
    for (const r of rows) {
      ...
      r.el.style.display = (byJob && byFame) ? '' : 'none';
    }
  };
  ...
  const sortRows = () => {
    ...
    if (key === 'dom') {
      // revert to original relative order for visible ones by appending in that order
      ordered = originalOrder.filter(el => el.style.display !== 'none').map(el => rows.find(r=>r.el===el));
    } else {
      const cmp = (a,b) => {
        let va, vb;
  ...
  // Small UX: press Enter in fame expr -> apply
  $('#fp-fameexpr').addEventListener('keydown', (e) => {
    if (e.key === 'Enter') { applyAll(); }
  });

  console.log('%c[필터 보기] 패널이 활성화되었습니다.', 'color:#7dd3fc');
})();
